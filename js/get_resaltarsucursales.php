...
$idsucursalsel = $_POST["idsucursal"];
...
$lon2 = $longitud - 0.075;
  ...
  $selectSucursales = mysqli_query($linkconx, "SELECT id, latitude, longitude FROM coordenadas WHERE latitude<'$lat1' AND latitude>'$lat2' AND longitude<'$lon1' AND longitude>'$lon2' ORDER BY id ASC ");
    ...
    while ($result = mysqli_fetch_array($selectSucursales)) {
      ...
      if ($idsucursalsel == $idSucursal) {
        // code...
        $listaSucursales .= '<li id="'.$idSucursal.'">
                                <span class="btnLista" id="btn_'.$idSucursal.'"><span class="numLista">Distribuidor </span> <b>' . $resultSucursal['distribuidor'] . '</b></span>
                                <p><b>Sucursal: ' . $nombresucursal . '<br>
                                    ' . $resultdirecciones['direccion'] . '<br>
                                    Municipio: ' . $resultdirecciones['municipio'] . '<br>
                                    Horario: ' . $resultSucursal['horario'] . '<br>
                                    Teléfono: <a href="tel:' . $resultSucursal['telefono'] . '">' . $resultSucursal['telefono'] . '</a></b>
                                </p>
                            </li>'; 
                            $i++;
        ...
        $listaSucursales .= '<li id="'.$idSucursal.'">
                                <span class="btnLista" id="btn_'.$idSucursal.'" onclick="mostrarenMapa('.$idSucursal.', '.$result['latitude'].', '.$result['longitude'].', \''.$nombresucursal.'\', \'' . $resultSucursal['familias'] . '\', \'' . $resultdirecciones['direccion'] . '\', \'' . $resultdirecciones['municipio'] . '\', \'' . $resultSucursal['horario'] . '\', \'' . $resultSucursal['telefono'] . '\', \'' . $resultSucursal['distribuidor'] . '\')"><span class="numLista">Distribuidor </span> ' . $resultSucursal['distribuidor'] . '</span>

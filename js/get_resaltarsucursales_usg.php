...
$lon2 = $longitud - 0.075;
  ...
  $selectSucursales = mysqli_query($linkconx, "SELECT * FROM sucursales_usg WHERE latitude<'$lat1' AND latitude>'$lat2' AND longitude<'$lon1' AND longitude>'$lon2' ORDER BY id ASC ");
    # code...
    $i=1;
    while ($result = mysqli_fetch_array($selectSucursales)) {
      ...
      if ($idsucursalsel == $idSucursal) {
        ...
        $listaSucursales .= '<li id="'.$idSucursal.'">
                                <span class="btnLista" id="btn_'.$idSucursal.'"><span class="numLista">Distribuidor </span> <b>' . $result['distribuidor'] . '</b></span>
                                <p><b>Sucursal: ' . $nombresucursal . '<br>
                                    ' . $result['direccion'] . '<br>
                                    Municipio: ' . $result['municipio'] . '<br>
                                    Horario: ' . $result['horario'] . '<br>
                                    Teléfono: <a href="tel:' . $result['telefono'] . '">' . $result['telefono'] . '</a></b>
                                </p>
                            </li>'; 
                            $i++;

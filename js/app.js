...
const infoWindow = new google.maps.InfoWindow();
  ...
  map.data.loadGeoJson('js/sucursales-usg-dic-07.json', {idPropertyName: 'storeid'});
  // Define the custom marker icons, using the store's "category".
  map.data.setStyle((feature) => {
    return {
      icon: {
        url: `img/icon_${feature.getProperty('category')}.png`,
        url: 'img/icono_ubicacion-usg.png',
...
function mostrarSucursales(datolat, datolong) {
  ...
  xhr_dataSucursales.open("POST", "js/get_sucursales_usg.php", true);
...
function mostrarSucursalespais(pais) {
  ...
  xhr_dataporpais.open("POST", "js/get_sucursalesporpais_usg.php", true);
...
function resaltarSucursales(datolat, datolong, idsucursal) {
  ...
  // AJAX
  var xhr_dataSucursales = new XMLHttpRequest();
  xhr_dataSucursales.open("POST", "js/get_resaltarsucursales_usg.php", true);
  xhr_dataSucursales.onload = function () {
    if (this.status == 200) {
      var res_listaSucursales = JSON.parse(this.response);
      if (res_listaSucursales.status == true) {
